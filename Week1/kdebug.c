...
print_stackframe(void) {
    ...
    uint32_t debug_ebp = read_ebp();
    uint32_t debug_eip = read_eip();
    //uint32_t* eip_addr = &debug_eip;
    for (int i = 0; i < STACKFRAME_DEPTH && debug_ebp != 0; i++) {
        uint32_t* ebp_addr = (uint32_t*)debug_ebp;
        cprintf("ebp:0x%08x eip:0x%08x args:", debug_ebp, debug_eip);
        uint32_t args[4];
        for (int j = 0; j < 4; j++) {
            args[j] = (ebp_addr + 2)[j];
            cprintf("0x%08x ", args[j]);
        }
        print_debuginfo(debug_eip - 1);
        //cprintf("\n");
        debug_ebp = ebp_addr[0];
        debug_eip = ebp_addr[1];
    }
    //后来看过答案之后才发现可以直接(uint_32*)进行转换
    //根据实验文档调整了下输出格式
